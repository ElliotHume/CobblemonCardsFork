    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand usedHand) {
        ...
        if (!level.isClientSide()) {
            ...
            if (isCustom) {
                ...
                boolean guaranteedGodPack = PlatformHelper.INSTANCE.hasGuaranteedGodPack(player);
                ...
                if (isGodPack) {
                    ...
                    if (guaranteedGodPack) {
                        PlatformHelper.INSTANCE.setGuaranteedGodPack(player, false);
                    }
                } else {
                    // Logique classique
                    for (int i = 0; i < 3; i++) {
            ...
            PlatformHelper.INSTANCE.sendToPlayer((ServerPlayer) player, new OpenBoosterPayload());
            PlatformHelper.INSTANCE.sendToPlayer((ServerPlayer) player, new GiveRewardPayload(displayRewards));
