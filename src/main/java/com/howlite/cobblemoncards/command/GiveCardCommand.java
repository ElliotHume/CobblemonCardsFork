...
public class GiveCardCommand {
    ...
    private static final SuggestionProvider<CommandSourceStack> POKEMON_SUGGESTIONS = (context, builder) -> {
        ...
        for (String base : baseNames) {
            names.add(base);
            if (alolan.contains(base)) names.add(base + "_alolan");
            if (galarian.contains(base)) names.add(base + "_galarian");
            if (hisuian.contains(base)) names.add(base + "_hisuian");
    ...
    private static final SuggestionProvider<CommandSourceStack> RARITY_SUGGESTIONS = (context, builder) -> 
            SharedSuggestionProvider.suggest(RARITIES, builder);

    private static final SuggestionProvider<CommandSourceStack> STAT_SUGGESTIONS = (context, builder) -> 
            SharedSuggestionProvider.suggest(Arrays.stream(CardStat.values()).map(CardStat::getSerializedName), builder);

    private static final SuggestionProvider<CommandSourceStack> BACKGROUND_SUGGESTIONS = (context, builder) ->
            SharedSuggestionProvider.suggest(BACKGROUNDS, builder);

    private static final SuggestionProvider<CommandSourceStack> EFFECT_SUGGESTIONS = (context, builder) -> 
            SharedSuggestionProvider.suggest(EFFECTS, builder);
        ...
        dispatcher.register(Commands.literal("cobblecard")
            ...
            .then(Commands.literal("carddex")
                .then(Commands.literal("fill")
                    .executes(context -> {
                        try {
                            ServerPlayer player = context.getSource().getPlayerOrException();
                            java.util.List<String> all = com.cobblemon.mod.common.api.pokemon.PokemonSpecies.getImplemented().stream()
                                .map(s -> s.getName().toLowerCase())
                                .distinct()
                                .toList();
                            java.util.List<String> attachment = player.getAttachedOrCreate(com.howlite.cobblemoncards.attachment.PlayerDataAttachments.DISCOVERED_CARDS, java.util.ArrayList::new);
                            attachment.clear();
            ...
            .then(Commands.literal("fill_cabinet")
                .executes(context -> {
                    try {
                        ServerPlayer player = context.getSource().getPlayerOrException();
                        net.minecraft.world.phys.BlockHitResult hitResult = (net.minecraft.world.phys.BlockHitResult) player.pick(5.0D, 0.0F, false);
                        ...
                        if (be instanceof com.howlite.cobblemoncards.block.entity.CardCabinetBlockEntity cabinetBE) {
                            ...
                            com.howlite.cobblemoncards.component.CardData dummyData = new com.howlite.cobblemoncards.component.CardData(
                                "pikachu", false, "common", com.howlite.cobblemoncards.component.CardStat.MOVEMENT_SPEED, 0.0f, 0, java.util.Optional.empty(), java.util.Optional.empty()
                            );
                        ...
                        .then(Commands.argument("shiny", BoolArgumentType.bool())
                            .then(Commands.argument("rarity", StringArgumentType.string())
                                ...
                                .then(Commands.argument("stat", StringArgumentType.string())
                                    ...
                                    .then(Commands.argument("stat_value", FloatArgumentType.floatArg(0f))
                                        .then(Commands.argument("background", StringArgumentType.string())
                                            ...
                                            .then(Commands.argument("effect", StringArgumentType.string())
                                                ...
                                                .executes(GiveCardCommand::execute)
                                            )
                                            .executes(context -> executeInternal(context, false, true))
                                        )
                                        .executes(context -> executeInternal(context, false, false))
                                    )
                                )
                                .executes(context -> executeInternal(context, true, false))
                            )
                        )
                    )
                )
