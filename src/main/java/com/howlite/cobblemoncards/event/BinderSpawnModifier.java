        ...
        ServerEntityEvents.ENTITY_LOAD.register((entity, world) -> {
            if (entity instanceof PokemonEntity pokemonEntity) {
                ...
                if (pokemon.getOwnerUUID() == null) {
                    Player nearestPlayer = world.getNearestPlayer(entity, 64.0);

                    if (nearestPlayer instanceof ServerPlayer serverPlayer) {
                        TrinketsApi.getTrinketComponent(serverPlayer).ifPresent(trinketComponent -> {
                            trinketComponent.forEach((slotReference, itemStack) -> {
                                String slotId = slotReference.inventory().getSlotType().getName();
                                if ((slotId.equals("belt") || slotId.equals("legs")) && itemStack.getItem() instanceof BinderItem) {
                                    handleSpawnModification(pokemonEntity, pokemon, itemStack);
                                }
                            });
                        });
                    }
